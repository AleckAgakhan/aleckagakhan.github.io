export const missingAttrMsg = (tagName, attr) => `The required attribute <${tagName} ${attr}="..."> is missing or empty.`;
export const missingTemplateProviderMsg = (tagName) => `Unable to find tempalte provider: <${tagName}> can only be placed within shadow tree of a template provider element or its shadow descendants.`;
export const missionTemplateElementMsg = (tagName, attr, id) => `Unable to find the element specified by the reference <${tagName} ${attr}="${id}">`;
export const missingElementInsideTemplateMsg = (tagName, templateKey, elementId) => `<${tagName}> requires the element with the id '${elementId}' inside the template '${templateKey}'.`;
export const delay = (timemout = 0) => new Promise((resolve) => setTimeout(resolve, timemout));
...
export const getMonthDayString = (date) => months[date.getMonth()] + ' ' + date.getDate().toString();
